...
CBallsArray::CBallsArray(int max_balls)
{
    ...
    this->trap = new CTrap(200, 180, 1500, 50);
    this->trapInverted = new CTrapInverted(300, 250, 1500, 50);
}
...
for (int i = 0; i < count; i++) {
    ...
    // Корректировка скорости с учетом гравитации.
    // Постоянно изменяется скорость шаров по оси ординат.
    // Тем самым скорость шаров стримится к "+ бесконечности", т.е.
    // шары стримятся упасть вниз.
    for (int i = 0; i < count; i++) {
    ...
    // Корректировка с учетом ловушки.
    for (int i = 0; i < count; i++) {
        this->trap->Effect(balls[i]);
    }

    // Корректировка с учетом ловушки (обратной).
    for (int i = 0; i < count; i++) {
        this->trapInverted->Effect(balls[i]);
    }
}
